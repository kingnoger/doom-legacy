...
void SCR_SetMode (void)
{
    ...
    } else I_Error ("unknown bytes per pixel mode %d\n", vid.bpp);
...
#ifdef USEASM
  // VB: self modifying asm code: set scanline widths
  CONS_Printf("vid.width = %d\n", vid.width);
  // VB: FIXME! HORRIBLE THINGS CAN HAPPEN!
  ASM_PatchRowBytes(vid.width);
#endif
  // VB: load palette from wad, set up screens[]
  V_Init();
...
//
int I_GetVideoModeForSize(int w, int h);  //vid_vesa.c
